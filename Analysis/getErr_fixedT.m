...
reachDir_absolute =reachDir*180/pi - 90; 
ibad = find(reachDir_absolute < -180);
reachDir_absolute(ibad) = reachDir_absolute(ibad)+360;
ibad = find(reachDir_absolute > 180);
reachDir_absolute(ibad) = reachDir_absolute(ibad)-360;

% reach direction relative to goal
reachDir_goal = reachDir_absolute+data.goalAng';
ibad = find(reachDir_goal < -180);
reachDir_goal(ibad) = reachDir_goal(ibad)+360;
ibad = find(reachDir_goal > 180);
reachDir_goal(ibad) = reachDir_goal(ibad)-360;

% reach direction relative to symbol
reachDir_symb = reachDir_absolute+data.symbAng';
ibad = find(reachDir_symb < -180);
reachDir_symb(ibad) = reachDir_symb(ibad)+360;
ibad = find(reachDir_symb > 180);
reachDir_symb(ibad) = reachDir_symb(ibad)-360;

data.reachDir_goal = reachDir_goal;
data.reachDir_symb = reachDir_symb;
data.reachDir_absolute = reachDir_absolute;%_absolute;
data.iInit = iInit;
data.iDir = iDir;
data.iEnd = iEnd;
data.tanVel = tanVelocity;
data.RT = data.iInit/130 - data.targ_appear_time'/1000 - .1;
data.pkVel = pkVel;
